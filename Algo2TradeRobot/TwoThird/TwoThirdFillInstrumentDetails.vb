    ...
    Public Async Function GetInstrumentData(ByVal allInstruments As IEnumerable(Of IInstrument), ByVal bannedStock As List(Of String)) As Task
        If allInstruments IsNot Nothing AndAlso allInstruments.Count > 0 Then
            ...
            If nfoInstruments IsNot Nothing AndAlso nfoInstruments.Count > 0 Then
                ...
                For Each runningCurrentInstrument In currentNFOInstruments
                    _cts.Token.ThrowIfCancellationRequested()
                    If runningCurrentInstrument.RawExchange.ToUpper = "NFO" AndAlso (bannedStock Is Nothing OrElse
                        bannedStock IsNot Nothing AndAlso Not bannedStock.Contains(runningCurrentInstrument.RawInstrumentName)) Then
                        Dim futureHistoricalCandlesJSONDict As Dictionary(Of String, Object) = Await GetHistoricalCandleStickAsync(runningCurrentInstrument.InstrumentIdentifier, tradingDay.AddDays(-10), tradingDay.AddDays(-1), TypeOfData.EOD).ConfigureAwait(False)
                        If futureHistoricalCandlesJSONDict IsNot Nothing AndAlso futureHistoricalCandlesJSONDict.Count > 0 Then
                            Dim futureEODPayload As Dictionary(Of Date, OHLCPayload) = Await GetChartFromHistoricalAsync(futureHistoricalCandlesJSONDict, runningCurrentInstrument.TradingSymbol).ConfigureAwait(False)
                            If futureEODPayload IsNot Nothing AndAlso futureEODPayload.Count > 0 Then
                                ...
                                If lastDayPayload.ClosePrice.Value >= 80 AndAlso lastDayPayload.ClosePrice.Value <= 1500 Then
                                    Dim rawCashInstrument As IInstrument = allInstruments.ToList.Find(Function(x)
                                                                                                          Return x.TradingSymbol = runningCurrentInstrument.RawInstrumentName
                                                                                                      End Function)
                                    ...
                                    If allStockData IsNot Nothing AndAlso allStockData.Rows.Count > 0 Then
                                        For i = 0 To allStockData.Rows.Count - 1
                                            If i < 5 Then
                                                ...
                                                'Else
                                                '    allStockData.Rows(i)(0) = todayStockList(i - 5).Remove(todayStockList(i - 5).Count - 8)
                                            End If
                                        Next
