...
#End Region
    ...
    Protected Overrides Async Function IsTriggerReceivedForPlaceOrderAsync(forcePrint As Boolean) As Task(Of List(Of Tuple(Of ExecuteCommandAction, PlaceOrderParameters, String)))
        ...
        Try
            If runningCandlePayload IsNot Nothing AndAlso runningCandlePayload.PreviousPayload IsNot Nothing AndAlso Me.TradableInstrument.IsHistoricalCompleted Then
                If _signalCandle Is Nothing Then
                    ...
                    Dim potentialLongEntryQuantity As Integer = CalculateQuantityFromInvestment(potentialLongEntryPrice, 30, userSettings.InstrumentsData(Me.TradableInstrument.TradingSymbol).Capital, userSettings.InstrumentsData(Me.TradableInstrument.TradingSymbol).AllowCapitalToIncrease)
                    Dim potentialShortEntryQuantity As Integer = CalculateQuantityFromInvestment(potentialShortEntryPrice, 30, userSettings.InstrumentsData(Me.TradableInstrument.TradingSymbol).Capital, userSettings.InstrumentsData(Me.TradableInstrument.TradingSymbol).AllowCapitalToIncrease)
            ...
            Else
                ...
                If currentTick.LastPrice >= potentialLongEntryPrice + longEntryBuffer Then
                    ...
                    If _firstTradeQuantity = Integer.MinValue Then
                        If userSettings.InstrumentsData(Me.TradableInstrument.TradingSymbol).Quantity <> Integer.MinValue Then
                        ...
                        Else
                            If Me.TradableInstrument.InstrumentType = IInstrument.TypeOfInstrument.Cash Then
                                quantity = CalculateQuantityFromInvestment(potentialEntryPrice, 13, userSettings.InstrumentsData(Me.TradableInstrument.TradingSymbol).Capital, userSettings.InstrumentsData(Me.TradableInstrument.TradingSymbol).AllowCapitalToIncrease)
                            Else
        ...
        If parameters IsNot Nothing Then
            ...
            If currentSignalActivities IsNot Nothing AndAlso currentSignalActivities.Count > 0 Then
                ...
                ElseIf currentSignalActivities.FirstOrDefault.EntryActivity.RequestStatus = ActivityDashboard.SignalStatusType.Completed Then
                    If ret Is Nothing Then ret = New List(Of Tuple(Of ExecuteCommandAction, PlaceOrderParameters, String))
                    ret.Add(New Tuple(Of ExecuteCommandAction, PlaceOrderParameters, String)(ExecuteCommandAction.Take, parameters, "Condition Satisfied"))
                ElseIf currentSignalActivities.FirstOrDefault.EntryActivity.RequestStatus = ActivityDashboard.SignalStatusType.Cancelled Then
                    If ret Is Nothing Then ret = New List(Of Tuple(Of ExecuteCommandAction, PlaceOrderParameters, String))
                    ret.Add(New Tuple(Of ExecuteCommandAction, PlaceOrderParameters, String)(ExecuteCommandAction.Take, parameters, "Condition Satisfied"))
                Else
                    If ret Is Nothing Then ret = New List(Of Tuple(Of ExecuteCommandAction, PlaceOrderParameters, String))
        ...
        If OrderDetails IsNot Nothing AndAlso OrderDetails.Count > 0 Then
            For Each runningOrderID In OrderDetails.Keys
                ...
                If bussinessOrder.SLOrder IsNot Nothing AndAlso bussinessOrder.SLOrder.Count > 0 Then
                    ...
                    If bussinessOrder.ParentOrder.TransactionType = IOrder.TypeOfTransaction.Buy Then
                        If targetReachedForBreakevenMovement AndAlso userSettings.StoplossMovementToBreakeven Then
    ...
    Protected Overrides Async Function ForceExitSpecificTradeAsync(order As IOrder, reason As String) As Task
        If order IsNot Nothing AndAlso Not order.Status = IOrder.TypeOfStatus.Complete AndAlso
            ...
            If exitOrderResponse IsNot Nothing AndAlso exitOrderResponse.AllOrder IsNot Nothing AndAlso exitOrderResponse.AllOrder.Count > 0 Then
                ...
                'Adding delay for single capital usage
                Dim userInputs As TwoThirdUserInputs = Me.ParentStrategy.UserSettings
                If userInputs.Delay > 0 Then
                    If reason.ToUpper = "STOPLOSS REACHED" OrElse
                        reason.ToUpper = "FORCE EXIT ORDER FOR REVERSE ENTRY" Then
                        Await Task.Delay(userInputs.Delay).ConfigureAwait(False)
                        While _lastTick.Timestamp.Value = Me.TradableInstrument.LastTick.Timestamp.Value
    ...
    Private Async Function GenerateTelegramMessageAsync(ByVal message As String) As Task
        ...
        If userInputs.TelegramAPIKey IsNot Nothing AndAlso Not userInputs.TelegramAPIKey.Trim = "" AndAlso
            ...
            Dim telegramID As String = Nothing
            If Me.TradableInstrument.InstrumentType = IInstrument.TypeOfInstrument.Cash Then
                telegramID = userInputs.TelegramCashChatID
            Else
                telegramID = userInputs.TelegramChatID
            End If
            Using tSender As New Utilities.Notification.Telegram(userInputs.TelegramAPIKey.Trim, telegramID, _cts)
